...
class Controller {
    ...
    async start() {
        ...
        this.mqtt.publish('bridge/meta/name',         'Zigbee2mqtt', {retain: true, qos: 0});
        this.mqtt.publish('bridge/controls/state',         'online', {retain: true, qos: 0});
        this.mqtt.publish('bridge/controls/state/meta/type', 'text', {retain: true, qos: 0});
    ...
    async publishEntityState(IDorName, payload, stateChangeReason=null) {
        ...
        if (Object.entries(messagePayload).length) {
            ...
            if (output === 'attribute_and_json' || output === 'json') {
                ...
                await this.mqtt.publish(`${resolvedEntity.name}/meta/name`, `ZigBee ${resolvedEntity.name}`, {retain: true});
                for (var prop in messagePayload) {
                    if (prop !== 'device') {
                        await this.mqtt.publish(`${resolvedEntity.name}/controls/${prop}`, `${messagePayload[prop]}`, {retain: true});
                        await this.mqtt.publish(`${resolvedEntity.name}/controls/${prop}/meta/type`, 'text', {retain: true});
                    } else {
                        for (var prop2 in messagePayload.device) {
                            await this.mqtt.publish(`${resolvedEntity.name}/controls/${prop2}`, `${messagePayload.device[prop2]}`, {retain: true});
                            await this.mqtt.publish(`${resolvedEntity.name}/controls/${prop2}/meta/type`, 'text', {retain: true});
                        }
                    }
                }
                if (resolvedEntity.definition) {
                    const {vendor, description, model} = resolvedEntity.definition;
                    await this.mqtt.publish(`${resolvedEntity.name}/controls/model`, `${description} (${model})`, {retain: true});
                    await this.mqtt.publish(`${resolvedEntity.name}/controls/model/meta/type`, 'text', {retain: true});
                    await this.mqtt.publish(`${resolvedEntity.name}/controls/model/meta/readonly`, '1', {retain: true});
                    await this.mqtt.publish(`${resolvedEntity.name}/controls/manufacturer`, vendor, {retain: true});
                    await this.mqtt.publish(`${resolvedEntity.name}/controls/manufacturer/meta/type`, 'text', {retain: true});
                    await this.mqtt.publish(`${resolvedEntity.name}/controls/manufacturer/meta/readonly`, '1', {retain: true});
                }
            }
