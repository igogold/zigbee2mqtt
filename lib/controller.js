        ...
        if (Object.entries(messagePayload).length) {
            if (settings.get().experimental.output === 'attribute_and_json') {
                ...
                for (var prop in messagePayload) {
                    if (prop !== 'device') {
                        await this.mqtt.publish(`${resolvedEntity.name}/controls/${prop}`, `${messagePayload[prop]}`, options);
                        ...
                        for (var prop2 in messagePayload.device) {
                            await this.mqtt.publish(`${resolvedEntity.name}/controls/${prop2}`, `${messagePayload.device[prop2]}`, options);
                ...
                if (resolvedEntity.definition) {
                    const {vendor, description, model} = resolvedEntity.definition;
                    await this.mqtt.publish(`${resolvedEntity.name}/controls/model`, `${description} (${model})`, options);
                    ...
                    await this.mqtt.publish(`${resolvedEntity.name}/controls/manufacturer`, vendor, options);
