    ...
    async publishEntityState(IDorName, payload, stateChangeReason=null) {
        ...
        if (Object.entries(messagePayload).length) {
            if (settings.get().experimental.output === 'attribute_and_json') {
                ...
                await this.mqtt.publish(entity.name, JSON.stringify(messagePayload), options);
                await this.mqtt.publish(`${entity.name}/meta/name`, entity.name, {retain: true});
                for (var prop in messagePayload) {
                    if (prop !== 'device') {
                        await this.mqtt.publish(`${entity.name}/controls/${prop}`, `${messagePayload[prop]}`, options);
                        await this.mqtt.publish(`${entity.name}/controls/${prop}/meta/type`, 'text', {retain: true});
                    } else {
                        for (var prop2 in messagePayload.device) {
                            await this.mqtt.publish(`${entity.name}/controls/${prop2}`, `${messagePayload.device[prop2]}`, options);
                            await this.mqtt.publish(`${entity.name}/controls/${prop2}/meta/type`, 'text', {retain: true});
                        }
                    }
                }
                if (entity.mapped) {
                    const {vendor, description, model} = entity.mapped;
                    await this.mqtt.publish(`${entity.name}/controls/model`, `${description} (${model})`, options);
                    await this.mqtt.publish(`${entity.name}/controls/model/meta/type`, 'text', {retain: true});
                    await this.mqtt.publish(`${entity.name}/controls/model/meta/readonly`, '1', {retain: true});
                    await this.mqtt.publish(`${entity.name}/controls/manufacturer`, vendor, options);
                    await this.mqtt.publish(`${entity.name}/controls/manufacturer/meta/type`, 'text', {retain: true});
                    await this.mqtt.publish(`${entity.name}/controls/manufacturer/meta/readonly`, '1', {retain: true});
                }
            } else {
                /* istanbul ignore else */
