...
class Availability extends Extension {
    ...
    onDeviceRemoved(resolvedEntity) {
        this.mqtt.publish(`${resolvedEntity.name}/controls/availability`, null, {retain: true, qos: 0});
    ...
    publishAvailability(device, available, force=false) {
        ...
        const topic = `${name}/controls/availability`;
        ...
        if (this.state[ieeeAddr] !== available || force) {
            ...
            this.mqtt.publish(`${name}/meta/name`, name, {retain: true, qos: 0});
        }
    }
