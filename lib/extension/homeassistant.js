...
class HomeAssistant extends Extension {
    ...
    discover(resolvedEntity, force=false) {
        ...
        this.getConfigs(resolvedEntity).forEach((config) => {
            ...
            // Set unique_id
            payload.unique_id = `${resolvedEntity.settings.ID}_${config.object_id}_wirenboard`;

            // Attributes for device registry
            payload.device = this.getDevicePayload(resolvedEntity);

            // Availability payload
            payload.availability = [{topic: `${settings.get().mqtt.base_topic}/bridge/controls/state`}];
            if (settings.get().advanced.availability_timeout) {
                payload.availability.push({topic: `${settings.get().mqtt.base_topic}/${friendlyName}/controls/availability`});
            }
