...
class BridgeLegacy extends Extension {
    ...
    async publish() {
        ...
        await this.mqtt.publish('bridge/controls/version',               payload.version,          {retain: true});
        await this.mqtt.publish('bridge/controls/version/meta/type',     'text',                   {retain: true});
        await this.mqtt.publish('bridge/controls/version/meta/readonly', '1',                      {retain: true});
        await this.mqtt.publish('bridge/controls/log_level',             payload.log_level,        {retain: true});
        await this.mqtt.publish('bridge/controls/log_level/meta/type',   'text',                   {retain: true});
        await this.mqtt.publish('bridge/controls/permit_join',           `${payload.permit_join}`, {retain: true});
        await this.mqtt.publish('bridge/controls/permit_join/meta/type', 'text',                   {retain: true});
    }
