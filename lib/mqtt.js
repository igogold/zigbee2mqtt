...
class MQTT extends events.EventEmitter {
    ...
    async connect() {
        ...
        const options = {
            will: {
                topic: `${settings.get().mqtt.base_topic}/bridge/controls/state`,
                payload: 'offline',
                retain: true,
            },
        };
        ...
        return new Promise((resolve) => {
            ...
            this.client.on('connect', () => {
                ...
                this.publish('bridge/meta/name',                 'Zigbee', {retain: true});
                this.publish('bridge/controls/state',            'online', {retain: true});
                this.publish('bridge/controls/state/meta/type',  'text',   {retain: true});
                resolve();
            });
    ...
    async disconnect() {
        ...
        await this.publish('bridge/controls/state',           'offline', {retain: true});
        await this.publish('bridge/controls/state/meta/type', 'text',    {retain: true});
    ...
    log(type, message, meta=null) {
        ...
        this.publish('bridge/controls/log',           JSON.stringify(payload), {retain: false});
        this.publish('bridge/controls/log/meta/type', 'text',                  {retain: true });
    }
}
