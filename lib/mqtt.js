...
class MQTT extends events.EventEmitter {
    ...
    async connect() {
        ...
        return new Promise((resolve) => {
            ...
            this.client.on('connect', () => {
                ...
                self.publish('bridge/controls/state', 'online', {retain: true, qos: 0});
                self.publish('bridge/controls/state/meta/type', 'text', {retain: true, qos: 0});
                self.publish('bridge/meta/name', 'Zigbee2mqtt', {retain: true, qos: 0});
                resolve();
            });
    ...
    async disconnect() {
        ...
        await this.publish('bridge/controls/state', 'offline', {retain: true, qos: 0});
        await this.publish('bridge/controls/state/meta/type', 'text', {retain: true, qos: 0});
